<div>
    ...
    <form wire:submit.prevent="addTask" class="mb-8 group">
        <div class="flex flex-col gap-3 p-3 border border-gray-200 rounded-lg focus-within:border-gray-400 transition-all shadow-sm">
            ...
            <div class="flex flex-wrap justify-between items-center border-t border-gray-50 pt-2 gap-2">
                <div class="flex gap-2">
                    <select wire:model="project_id" class="text-xs border-none bg-gray-50 rounded px-2 py-1 outline-none text-gray-600 cursor-pointer">
                    ...
                    <input type="date" wire:model="due_date" 
                        class="text-xs border-none bg-gray-50 rounded px-2 py-1 outline-none text-gray-600 cursor-pointer">
                </div>

                <button type="submit" class="bg-[#dc4c3e] hover:bg-[#b53d32] text-white text-xs font-bold px-4 py-1.5 rounded-md transition-colors shadow-sm">
                    Thêm nhiệm vụ
                </button>
            </div>
        </div>
                ...
                <div class="flex-1 flex items-center justify-between">
                    <div>
                        ...
                        <div class="flex gap-3 items-center mt-0.5">
                            ...
                            @if($task->due_date)
                                <span class="text-[10px] font-medium flex items-center gap-1 {{ $task->due_date->isPast() && !$task->is_completed ? 'text-red-500' : 'text-gray-400' }}">
